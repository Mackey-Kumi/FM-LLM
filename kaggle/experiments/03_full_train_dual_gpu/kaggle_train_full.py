"""
FM-LLM: full ETTh1 training run at the paper's true batch size (256, Table 2),
using Kaggle's T4x2 accelerator with real dual-GPU data parallelism (128+128
split per step) -- the configuration proven to fit in the batch=256 memory
test (peak ~12.3GB/GPU out of 15.6GB, vs. OOM on any single GPU).

Same verified architecture throughout: chronological split, Fourier Embedding
Module, frozen Llama-3.2-1B, FAN-MoE decoder, hybrid time-frequency loss +
sequence balance loss. Checkpoints the best validation model to
/kaggle/working/ so it's downloadable as the kernel's output.
"""
...
# ---------------------------------------------------------------------------
# 7. Full training loop: real epochs, early stopping, best-val checkpointing
# ---------------------------------------------------------------------------
NUM_EPOCHS = 30      # generous ceiling -- early stopping decides the real length
PATIENCE = 3          # paper's early-stopping patience
LAMBDA_BALANCE = 1.0  # Table 2 for ETTh1
LOG_EVERY = 5         # print progress every N batches within an epoch

best_val_loss = float("inf")
epochs_without_improvement = 0

print(f"\n{'=' * 70}")
print(f"TRAINING: up to {NUM_EPOCHS} epochs, batch_size={BATCH_SIZE}, patience={PATIENCE}")
print(f"{'=' * 70}\n")
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(
            ...
            f"{CHECKPOINT_DIR}/etth1_96_best.pt",
        )
...
print(f"Checkpoint saved at: {CHECKPOINT_DIR}/etth1_96_best.pt")
print(f"{'=' * 70}")
