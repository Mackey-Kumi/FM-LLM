...
run against all three checkpoints produced so far:
  - no_dropout:     etth1_96_best.pt            (03_full_train_dual_gpu)
  - dropout:        etth1_96_dropout_best.pt     (05_dropout_retrain, ceiling=30)
  - dropout_longer: etth1_96_dropout_longer_best.pt (07_dropout_retrain_longer,
                     ceiling=60, early-stopped naturally at epoch 35)

Identical in every other respect to 06_rollout_eval -- same rollout
mechanics, same test windowing, same architecture. Only the checkpoints
dict grew a third entry, to get one consistent three-way comparison table
instead of a second isolated run.
...
nn.Dropout carries no parameters or buffers, so it's absent from all three
...
mode here so it's inactive) loads all three checkpoints correctly.
...
# ---------------------------------------------------------------------------
# 4. Autoregressive rollout logic -- identical to 06_rollout_eval
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    CHECKPOINT_LABELS = ("no_dropout", "dropout", "dropout_longer")
    checkpoints = {
        "no_dropout": build_and_load("etth1_96_best.pt"),
        "dropout": build_and_load("etth1_96_dropout_best.pt"),
        "dropout_longer": build_and_load("etth1_96_dropout_longer_best.pt"),
    }
