"""
FM-LLM: ETTh1 retrain with dropout added (Table 2: dropout=0.2), otherwise
identical to 03_full_train_dual_gpu -- same batch=256, T4x2 DataParallel
split, lr=2e-4, early stopping (patience=3), 12/4/4-month chronological
split. Dropout is the only variable that changed versus the existing
etth1_96_best.pt checkpoint, so any difference in test MSE/MAE is
attributable to it.

Dropout placement follows Fig. 2's block diagrams: one Dropout in the
Fourier Embedding Module (before FAN), two in each Fourier (shared) expert
(before FAN and before the final projection). Routed experts are left
unchanged -- the paper describes them as "standard feed-forward networks...
composed of two linear layers" with no dropout mentioned.
...
tests/test_dropout_architecture.py) without triggering CUDA asserts, HF
downloads, or the training loop -- on Kaggle it still runs exactly as
before, since Kaggle executes this file as the main script.
"""
...
#    (tests/test_dropout_architecture.py) without a GPU, HF token, or
#    network access. Kaggle runs this file directly, so __name__ ==
#    "__main__" there and everything below still executes exactly as before.
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                ...
                f"{CHECKPOINT_DIR}/etth1_96_dropout_best.pt",
            )
    ...
    print(f"Checkpoint saved at: {CHECKPOINT_DIR}/etth1_96_dropout_best.pt")
    print(f"{'=' * 70}")
