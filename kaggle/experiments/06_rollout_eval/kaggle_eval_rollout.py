...
run against both the no-dropout checkpoint (etth1_96_best.pt, from
03_full_train_dual_gpu) and the dropout checkpoint (etth1_96_dropout_best.pt,
from 05_dropout_retrain).
...
appended and the oldest token is dropped, matching the fixed lookback
length used everywhere else in the paper.
...
nn.Dropout carries no parameters or buffers, so it's absent from both
checkpoints' state_dicts regardless of whether it was present during
training -- one model definition (with dropout, but always run in eval()
mode here so it's inactive) loads both checkpoints correctly.
...
tests/test_rollout_logic.py) without a GPU, HF token, or network access.
Kaggle runs this file directly, so everything below still executes as a
normal script there.
"""
...
DROPOUT = 0.2  # inactive under eval(), kept only so both checkpoints' state_dicts load cleanly

# ---------------------------------------------------------------------------
# 1. Fourier Embedding Module (paper Eq. 7-11) -- identical to
#    05_dropout_retrain/kaggle_train_full.py
# ---------------------------------------------------------------------------
...
#    05_dropout_retrain/kaggle_train_full.py, minus the training-only
...
# ---------------------------------------------------------------------------
# 4. Autoregressive rollout logic -- new for this experiment, pure/testable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Test-set windowing, generalized for the max horizon (720) needed here
# ---------------------------------------------------------------------------
...
#    (tests/test_rollout_logic.py) without a GPU, HF token, or network
#    access. Kaggle runs this file directly, so everything below still
#    executes exactly as a normal script there.
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    ...
    checkpoints = {
        "no_dropout": build_and_load("etth1_96_best.pt"),
        "dropout": build_and_load("etth1_96_dropout_best.pt"),
    }

    results = {}

    for label, (embedding_module, decoder) in checkpoints.items():
        print(f"\n{'=' * 70}")
        print(f"ROLLOUT EVAL: {label}")
        print(f"{'=' * 70}\n")
        ...
        with torch.no_grad():
            for i, (history_batch, future_batch) in enumerate(test_loader):
                history_tokens = history_batch.permute(0, 2, 1).unfold(-1, PATCH_LEN, PATCH_LEN).to(device)
    ...
    print(f"{'=' * 70}")
    print(f"{'horizon':>8} {'checkpoint':>12} {'MSE':>10} {'MAE':>10}")
    for h in HORIZONS:
        for label in ("no_dropout", "dropout"):
            mse, mae = results[label][h]
            print(f"{h:>8} {label:>12} {mse:>10.4f} {mae:>10.4f}")
    print(f"{'=' * 70}")
