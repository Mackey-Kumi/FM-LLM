"""
FM-LLM: ETTh1 retrain with instance (per-window) normalization added on top
of dropout, otherwise identical to 07_dropout_retrain_longer -- same
architecture, same batch=256 T4x2 DataParallel split, same lr=2e-4, same
patience=3, NUM_EPOCHS=60 ceiling, same 12/4/4-month split.

Motivation: the 08_rollout_eval_longer three-way comparison showed our
rollout error growing much faster across horizons (192->720: +26% MSE) than
the paper's own numbers do (+5% MSE) even on the best checkpoint so far --
the classic symptom of a model that isn't robust to each window's own local
level/scale. The paper explicitly borrows PatchTST's channel-independence
design (Section 3.2.1, citing PatchTST), and PatchTST's own architecture is
built around per-instance (RevIN-style) normalization, so it's plausible
this was inherited too even though the paper's text never calls it out as a
contribution in its own right.

This does NOT replace the existing global per-channel train-stats
normalization (train_norm/val_norm below) -- that stays exactly as before,
since that's what makes our MSE/MAE numbers comparable to the paper's
Table A.12 (which are reported on that same globally-normalized scale, not
raw physical units). Instance normalization is a second, *local* layer
purely inside the model's forward pass: each 672-step history window gets
its own mean/std computed from itself, normalized before entering the
model, and the model's prediction is denormalized back with that same
window's stats before the loss is computed against the (globally-
normalized, untouched) target. No model architecture changes at all --
FourierEmbeddingModule/FANMoEDecoder/etc. are byte-identical to 07;
instance_normalize/instance_denormalize are pure pre/post-processing
wrapped around forward_pass, kept outside the DataParallel-wrapped pipeline
since they're cheap elementwise ops with no learnable parameters.

Module-level code below the class/function definitions is guarded by
`if __name__ == "__main__":` so this file can be imported locally (by
tests/test_instance_norm.py) without triggering CUDA asserts, HF downloads,
or the training loop -- on Kaggle it still runs exactly as before, since
Kaggle executes this file as the main script.
"""
...
# ---------------------------------------------------------------------------
# 5. Instance (per-window) normalization -- new for this experiment,
#    pure/testable, no learnable parameters. Sits on top of the existing
#    global per-channel normalization, not in place of it.
# ---------------------------------------------------------------------------
def instance_normalize(history):
    ...
    each (batch, channel) row of history_norm has zero mean / unit std.
    """
...
# ---------------------------------------------------------------------------
# 6. Combined pipeline module + dataset
# ---------------------------------------------------------------------------
...
#    (tests/test_instance_norm.py) without a GPU, HF token, or network
#    access. Kaggle runs this file directly, so __name__ == "__main__"
#    there and everything below still executes exactly as a normal script.
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    ...
    def forward_pass(history_batch):
        ...
        history_norm, mean, std = instance_normalize(history)
        tokens = history_norm.unfold(-1, pred_len, pred_len)
        pred_patch_norm, top_indices, scores = pipeline(tokens)
        pred_patch = instance_denormalize(pred_patch_norm, mean, std)
        return pred_patch, top_indices, scores
    ...
    print(f"\n{'=' * 70}")
    print(f"TRAINING (dropout={DROPOUT}, instance norm): up to {NUM_EPOCHS} epochs, batch_size={BATCH_SIZE}, patience={PATIENCE}")
    print(f"{'=' * 70}\n")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                ...
                f"{CHECKPOINT_DIR}/etth1_96_instnorm_best.pt",
            )
    ...
    print(f"Checkpoint saved at: {CHECKPOINT_DIR}/etth1_96_instnorm_best.pt")
    print(f"{'=' * 70}")
