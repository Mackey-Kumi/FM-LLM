"""
FM-LLM: does the paper's true ETTh1 batch size (256, Table 2) fit on Kaggle's
T4x2 accelerator if we actually use BOTH GPUs via data parallelism (128+128),
instead of the single-GPU test that already confirmed OOM at ~15.3GB peak on
one T4?

Same verified architecture as kaggle_batch256_test.py (Fourier Embedding
Module, frozen Llama-3.2-1B, FAN-MoE decoder, hybrid loss). The only change:
the trainable embedding module + frozen backbone + decoder are wrapped in one
nn.Module and driven through nn.DataParallel(device_ids=[0, 1]), which splits
each batch across both GPUs and gathers the outputs back for loss computation.
"""
...
assert NUM_GPUS >= 2, "This test requires the T4x2 accelerator (2 GPUs) -- only found 1"
...
class RealLlamaBackbone(nn.Module):
    ...
    def forward(self, embedding_inputs):
        # NOTE: not self.llama.dtype -- that property does
        # next(p.dtype for p in self.parameters() if ...), and nn.DataParallel's
        # module replication breaks .parameters() iteration on replicas
        # (StopIteration). We already know the load dtype, so use it directly.
...
# ---------------------------------------------------------------------------
# 6. Combined pipeline module -- wrapped in DataParallel so a single forward()
#    call splits the batch across both T4s and gathers results back to cuda:0
# ---------------------------------------------------------------------------
class FullPipeline(nn.Module):
    def __init__(self):
        super().__init__()
        self.embedding_module = FourierEmbeddingModule(patch_len=pred_len, mlp_dim=512, llm_dim=2048)
...
base_pipeline = FullPipeline().to(device)
...
# ---------------------------------------------------------------------------
# 7. THE EXPERIMENT: attempt real train steps at batch=256 split across 2 GPUs
# ---------------------------------------------------------------------------
NUM_TEST_BATCHES = 3

print(f"\n{'=' * 70}")
print(f"EXPERIMENT: {NUM_TEST_BATCHES} real train steps at batch_size={BATCH_SIZE}, split across {NUM_GPUS} GPUs")
print(f"{'=' * 70}\n")
...
try:
    for i, (history_batch, future_batch) in enumerate(train_loader):
        ...
        mem_report = "  ".join(
            f"gpu{g}_peak={torch.cuda.max_memory_allocated(g) / 1e9:.2f}GB" for g in range(NUM_GPUS)
        )
        print(f"step {i + 1}/{NUM_TEST_BATCHES}  loss={loss.item():.4f}  time={time.time() - t0:.2f}s  {mem_report}")
    ...
    print(f"\nCUDA OutOfMemoryError at batch_size={BATCH_SIZE} (split across {NUM_GPUS} GPUs)")
    print(f"Peak memory before failure: {mem_report}")
    print(f"\nRaw error:\n{e}")

print(f"\n{'=' * 70}")
print(f"VERDICT: batch_size={BATCH_SIZE} split across {NUM_GPUS}x T4 -> {result}")
if result == "SUCCESS":
    mem_report = "  ".join(
        f"gpu{g}_peak={torch.cuda.max_memory_allocated(g) / 1e9:.2f}GB" for g in range(NUM_GPUS)
    )
    print(f"Peak memory across {NUM_TEST_BATCHES} steps: {mem_report}")
print(f"{'=' * 70}")
